...
In this Chapter we expand upon this discussion computing 
...
multi-dimensional, 
multi-group radiative transfer simulation coupled to a \ac{HD} (or \ac{MHD}) 
...
$R_{\text{ph}}$, with effective emission temperature, $T_{\text{eff}}$, 
is evaluated with the Stefan-Boltzmann law, \ie, assuming the 
...
Notably, when a temperature drops and atoms become neutral, 
...
significantly dimmer in the case of LS220 $q=1.00$ model.
%
...
in $g$ and $z$ bands, and within $0.5\,$day margin in $k_s$ band. 
Notably, fit-informed \acp{LC} systematically underestimate $t_p$ in the 
$K_s$ band. 
%
The largest deviation is found for the 
model with $q=1.8$ and BLh \ac{EOS}. This highlights limitations of our 
fitting formula that is not able to reproduce \ac{PC} model properties well.
...
that about ${\sim}0.05\, M_{\odot}$ would be ejected in a form of secular winds. 
%
