...
%
As we mentioned in Sec.~\ref{sec:intro:remnant}, 
the newly born \ac{NS} remnants are not axisymmetric, 
displaying characteristic spiral arms in their density 
profile that extend outwards from the shock interface of the collided cores 
...
Formation of the spiral arms is a generic hydrodynamic effect, 
that was identified in the \ac{NR} simulations with polytropic 
...
modes depends on the physics input and not well understood. 
...
%
If a short-lived remnant is present, \ie, in LS220 $q=1.00$ model, 
...
%
We examine these dependencies more closely in Appendix~\ref{ch:stat}, Sec.~\ref{sec:stat:remdisk}. \red{Maybe move it also here? in a separate section?}
%% -- 
...
This is the case for the previously discussed 
...
We employ the geodesic criterion (See Sec.~\ref{sec:bns_sims:method:ejecta}) 
to flag the \ac{DE}, extracting its properties at 
$R=294\,$km from the remnant (unless specified otherwise).
...
without subgrid turbilence (See Tab.~\ref{tab:sims}) we observe 
...
%% --- Overal dataseamble
In total $324$ \ac{NR} models are collected.
For each of which we compute the reduced tidal parameter, $\tilde{\Lambda}$, 
Eq.~\eqref{eq:intro:Lambda} \red{MIGT BE MISSING}, 
solving the \ac{TOV} equations for the corresponding 
...
polynomials of $q$ and $\tilde{\Lambda}$ of first and second order,
$P_2^1(q,\tilde{\Lambda})$ and $P_2^2(q,\tilde{\Lambda})$ respectively. 
The results of this analysis are the following. 

%% --- Mej
For the ejecta mass we observe 
that the intrinsic scatter in the data hinders performance 
of any smooth fitting formula. The inclusion of \mr{} is required to capture the leading 
trends in data and the simple polynomial $P_2^2(q,\tilde{\Lambda})$ show a comparatively  
...
%
The best fitting formula to the $\avd$, among considered, is given by the 
second order two-parameter polynomial $P_2^2(q,\tilde\Lambda)$ that appears to be able to capture 
the leading trends in data. 
...
that when models of \DSrefset{} only are considered, it varies from $0.03$ found in very high 
\mr{} binaries, that produce cold, low-$Y_e$, tidal ejecta \citep{Bernuzzi:2020tgt} to $0.27$ in $q\sim1$ 
...
However, it is difficult statistically analyze. For the sake of simplicity 
...
%
However, larger sample of ab-initio simulations with complete physics and 
