...
In this chapter we preset the results of postprocessing of our 
...
%
We begin by discussing the dynamics of the \pmerg{} remnant and 
its interactions with the surrounding disk in Sec.~\ref{sec:bns_sims:remdisk}. 
There, we focus on the disk evolution its final state. Additionally, we 
...
%
Fig.~\ref{fig:ang_mom_flux} shows the angular momentum is transported 
...
maximum mass supported by an \ac{EOS}, which leads to a \ac{PC} 
...
but neutron-rich $Y_e\sim0.1$, as it shielded from the neutrino irradiation. 
...
%
We observe, that during the formation, shocks and spiral waves raise the disk 
...
remnant and cooling. The disk mass as a function of time for a set of 
models is shown in Fig.~\ref{fig:disk_mass_evo}. 
%
If \ac{BH} forms, the only possible 
interaction, is the disk accretion onto a \ac{BH}, that leads to the disk depletion. 
...
This is the direct consequence of the thermal effects included in the BLh \ac{EOS} 
...
%as discussed above. 
Larger temperatures lead to lower rotational 
...
more \ac{BNS} merger simulations with advanced physics and consistent method 
...
%
The plot shows that the mass-weighted distribution of entropy and electron 
...
independent of \ac{EOS} and \mr{} while the latter peak is more pronounced 
...
total baryonic mass, $M_b$, and total angular momentum, $J$, (black solid line)  
...
%
At early \pmerg{}, before and substantial mass-loss could occur, the 
...
Notably, also the baryonic mass of the \pmerg{} remnant exceeds 
...
%
For BLh $q=1.00$ model, if the ejecta mass flux does change, 
...
The properties of \ac{SWW} are discussed in next section. 
...
baryonic mass. They also evolve toward the rigidly rotating equilibria but slower.
...
We discussed mechanisms behind \ac{DE} in Sec.~\ref{sec:intro:ejecta}. 
...
that its effect on \ac{DE} properties is rather week and comparable 
...
is not available, we compute it by solving \ac{TOV} equations\footnote{
    ...
    \ac{EOS} was developed by Sebastiano Bernuzzi, simplified version of which is 
...
that the the inclusion of this effect leads to the increase in \ac{DE} mass, in addition to the expected increase in average electron fraction. This is especially 
...
and binary parameters, specifically, a \mr{}.
...
\ac{DE} properties observed before, that we extend as follows. 
...
In order to analyze the low mass, very fast compoent of \ac{DE}, we 
...
is primarily of tidal origin and display on average lower velocities, 
...
similarly to what was observed for \ac{DE} overall (see Tab.~\ref{tab:sim}).
%% --- Table reference 
...
core first-\bnc{} ejection mechanism dominates, the fast ejecta tail is generally equatorial.
...
We report the overall properties of \ac{SWW} for all models with 
long-lived \ac{NS} remnants that were also evolved for a sufficiently long time 
in Tab.~\ref{tab:spiralwavewind}. 
%
The time evolution of the total mass of \ac{SWW} is shown in  Fig.~\ref{fig:mej:bern}.
We observe that in all simulations \ac{SWW} persist till the time these simulations 
were terminated without saturation.
...
%
The strongest \ac{SWW} are found in binaries with $q>1$, such as 
...
at lower \mr{}s. For instance, the mass flux of BLh* $q=1.66$ model 
...
by ${\sim}25$\% in the case of DD2 $q=1.00$ model, exceeding the finite 
resolution effects. 
%
...
configuration is dependent on the thermal effects. Higher temperatures lead to 
...
\ac{SWW} have high average electron fraction. Its overall broad distribution,  
...
simulations. Naturally, we expect the to be a part 
of \ac{SWW}, polar and with high $Y_e$. 
%
