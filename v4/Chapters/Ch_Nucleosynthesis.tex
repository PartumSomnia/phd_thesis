...
%
Here, we briefly discuss how numerous nuclear species, participating in 
thousands on nuclear reactions, are evolved using the novel \texttt{SkyNet} 
nuclear reaction network code, 
and how the process can be parameterized for quick 
...
discussed in Chapter~\ref{ch:bns_sims}.
...
reaction, rates is called \ac{NRN}. \ac{assert full version}
%
% BBN
...
of \citet{Lippuner:2018phd}, (see their Figure 3.1).
...
merger environment and is parameterized as a function of following ejecta 
...
\begin{itemize}
    ...
    \item electron fraction, $Y_e$, is sampled uniformly between $0.01$ and $0.5$ 
    ($Y_e>0.5$ is not favorable for \rproc{});
    \item specific entropy, $s$, is sampled lorarithmically 
    between $1$ and $100\, k_B/\text{baryon}$;
    \item expansion timescale, % $\tau$,  
    ...
    \item density profile is that if the homologous expanding fluid\footnote{
