...
\acp{RR} is called \ac{NRN}.% \ac{assert full version}
%
% BBN
...
% Abundances
Describing particle interactions and nuclide transmutations it is 
...
\texttt{SkyNet} solves the system of coupled, first-order, non-linear equations, 
...
%        long-lived \ac{NS} remnant.
        %
        \textit{Other five panels} show the total nucleosynthetic yields 
...
\end{equation}
gives the control over the dynamical timescale for the \rproc{} calculations 
...
temperature of $6\times10^9\,$K. The $\rho_0$ spans the range
 $(10^5 - 10^{12})\,$\gcm.
%
...
depends on the nuclear mass model, reaction rate and fission fragment distribution 
...
%$A=195$ equal solar.
%
From the plot we observe that when models with moderately soft 
\acp{EOS}, \eg, DD2 and BLh \acp{EOS}, that form long-lived 
remnants are considered, 
the final \rproc{} abundances in the ejecta are in a good agreement 
