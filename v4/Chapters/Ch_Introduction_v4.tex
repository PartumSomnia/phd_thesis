...
Wide range of possible types and properties of ejecta leads to a similarly broad range of 
...
Whether the ordered, large-scale \acp{MF} can form in a \pmerg{} environment 
...
This drives the neutron and proton fraction towards equilibrium, 
raising the $Y_e$.
%\red{paper says: 
...
component of \ac{DE}. It is more massive in binaries with 
...
velocity and mass, that is dependent on parameters of the binary and 
\ac{NS} \ac{EOS}. 
%
...
\rproc{} (\sproc{}).
%
The \sproc{} moves along the valey of stability\footnote{
...
A particular important quantity is the ratio of the number of free neutrons to 
the number of seed nuclides, neutron-to-seed ratio. 
...
%
However, at high entropy and low $Y_e$ the $3$rd peak elements can be synthesis as, 
while there are few seed nuclei, the neutron to seed ratio is high.
%
The main quantity defining the final abundances was shown is the 
...
viscous- and recombination-driven winds. These processes, are expected to take 
place over long timescales, ${\simeq}1\,$s. 
Studies have shown that this ejecta has a broad distribution of $Y_e$ and the \rproc{}
...
core \citep{Qian:1996xt} were suggested as a promising cite of \rproc{} \nuc{}  \citep{Woosley:2002,Wanajo:2006mq}.
...
initiated by the \ac{MRI} and accompanied by the % magnetorotational processes \eg, 
...
%
Additionally, if actinides are synthesized in \rproc{}, their decay produces, \eg,
...
generalize it with two main components, ``blue'' and ``red'' 
...
Notably, \ac{BNS} mergers that experience \ac{PC} are not expected to eject large 
amount of material and are generally thought of as being \ac{EM}-quiet 
\citep{Margalit:2017dij,Bauswein:2017vtn}. 
...
GRB050709 \citep{Jin:2016pnm}.
%GRB200522A \citep{WRONG}  Bruni:2021ilp
However the exact nature of observed signals were not well constrained. 
...
for at least some of \acp{SGRB}, it was dimmer than any other event of its class. 
...
mildly relativistic ejecta expanding into \ac{ISM} \citep{Nakar:2011cw}. 
Called, \ac{kN} afterglow, it is expected to peak in radio band and continue 
to be observable on a time scale of years 
after merger, providing a source of information that is largely free 
...
\acp{BNS} mergers are at the center of a variety of 
...
%    the resulted nucleosynthetic yields?
    Based on the abundance pattern in ejecta, can \ac{BNS} mergers be considered as 
...
and techniques we use to postprocess these simulations. 
%
...
Then, we apply a previously developed model to ejecta from our models 
...
numerically. Then we apply our new tool to the \ac{BNS} merger ejecta and 
