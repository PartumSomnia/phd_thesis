...
%% From Just
At the end of their evolution, pairs of massive stars undergo \acp{SN} explosions, leaving a pair of compact objects orbiting each other, if the progenitors were sufficiently massive.
Of particular interest is a pair of \acp{NS}, compact, heavy objects sustained 
...
loses energy and angular momentum to \acp{GW}. 
...
velocities. 
Additionally, the massive \pmerg{} remnant and gravitationally bound 
matter surrounding it are subjected to complex dynamical interactions, weak processes and magnetically induced 
...
astrophysical sites. Specifically, the abundance of free neutrons allows for the so-called 
...
The wide range of possible types and properties of ejecta leads to a similarly broad range of 
...
Furthermore, expanding into the \ac{ISM}, mildly relativistic ejecta is expected 
to generate non-thermal afterglow emission that can be observed years after a merger. 
...
Together with \ac{GW} emission, these signals allow us to study the processes occurring 
at mergers in great detail. Studies of \ac{BNS} mergers are crucial for 
...
as a source of \acp{GW} by \ac{LIGO}/Virgo, \GW{}. The unprecedented \ac{EM} 
follow-up campaign, spanning hundreds of observatories across the world, led to 
the identification of the merger \ac{EM} counterparts: \ac{kN}, \AT{} and 
\ac{SGRB}, \GRB{} 
%and compilation of the large amount of data
...
%
% And while \acp{GW} had shed light on the properties of cold,
% $\mathcal{O}(\text{keV})$, \ac{NS} matter 
...
Studies of \acp{GW} from \GW{} together with its \ac{EM} counterparts (so-called \ac{MM} studies) allowed to put constraints 
on the properties of matter at densities several times that of the 
nuclear matter, and investigate astrophysical implications of \ac{BNS} mergers \citep{Villar:2017wcc,Hajela:2019mjy,Radice:2018ozg,LIGOScientific:2017ync}.
...
\citep{Sekiguchi:2011zd,Wanajo:2014wha,Foucart:2015gaa,Palenzuela:2015dqa,
    %Sekiguchi:2016bjd,Kiuchi:2017zzg,
    Radice:2017zta,Fujibayashi:2017puw,Radice:2018pdn}.
%
These simulations, performed with \ac{NR} codes that took years or even 
decades to develop and test, are very computationally expensive, rare, and require detailed 
postprocessing and analysis. 
%
Moreover, the self-consistent modeling of a \ac{BNS} merger and its \ac{EM} counterparts 
is still beyond the reach of modern methods. Generally, the short-term (hundreds of milliseconds) 
...
and ultimately, the \ac{NS} \ac{EOS}. 
We expand upon this at the end of this Chapter, after we introduce the necessary 
concepts and discuss the current state of the field. 
...
\ac{BNS} mergers, which has been largely derived from \ac{NR} simulations.
...
\citep{Shibata:2019wef,Bernuzzi:2020tgt,Radice:2020ddv} 
...
%
For the sake of brevity we omit most of the technical details, 
...
The inspiral phase of a \ac{BNS} system is primarily governed by the emission 
...
Another approximation of the two-body dynamics in \ac{GR} is the \ac{EOB} formalism,
...
frequencies of \acp{GW} \citep{Damour:2009wj}.
%
...
The total mass is $M = M_A + M_B$, and the \mr{} $q=M_A/M_B\geq1$. 
%
...
From the analysis of \acp{GW}, \GW{} was interpreted as a \ac{BNS} merger with total mass of
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.80\textwidth]{Fig_3_Rad.pdf}
    \caption{
        Schematic picture of a \ac{BNS} merger with relative timescales. 
        After the inspiral \acp{NS} merger forming a massive object, the early evolution of 
        which is governed primarily by 
        hydrodynamics and by \ac{GW} emission. 
        %
        If a \ac{BH} does not form after ${\sim}10-20\,$ms, \ac{GW} emission subsides and other physical processes start affecting the evolution. These processes include angular momentum redistribution (due to viscous effects) and neutrino emission. 
        %
        This phase proceeds on a timescale of 
        seconds, but can be interacted at any point by a \ac{BH} formation. 
        %
        On even longer timescale the postmerger remnant continues to spin down due to \ac{MHD} effects. 
...
On a longer timescale, \ac{MHD} stresses contribute to angular momentum redistribution, 
...
The maximum values of temperatures and densities are reached at this point \citep{Perego:2019adq}. 
...
While shocks do form at the remnant \ac{NS} surface, inside the remnant 
the speed of sound is too high for shock formation. There,  
matter remains cold throughout the merger with 
the exception of the interface between 
two merged cores, where compression and shear dissipation raises the temperature 
to $\mathcal{O}(100\,\text{MeV})$. 
...
The newly born \ac{NS} remnant is not hydrodynamically stable. 
Its dynamics are characterized by $m=2$ bar- and $m=1$ one-armed-
...
%
This phase of evolution is sometimes referred to as the \ac{GW}-dominated \pmerg{} phase.


After the emission of \acp{GW} subsides, the \ac{NS} remnant may still have an excess 
in angular momentum and gravitational mass with respect to a cold, 
rigidly rotating equilibrium with the same baryonic mass \citep{Radice:2018xqa}. 
In other words, such an object is supported against collapse artificially, 
\eg, by differential rotation. 
%
Its subsequent evolution proceeds towards a more axisymmetric configuration 
...
as shown in Fig.~\ref{fig:intro:RadFig1}.  %, the mass-shedding limit. 
Depending on the lifetime of \pmerg{} remnants,
\ie, whether the collapse occurred during the 
\ac{GW}-dominated phase or not, we distinguish \textit{short-lived}
and \textit{long-lived} ones respectively. 
...
%
This is gravitationally bound matter that can be distinguished 
...
The evolution of the disk as it interacts with the \ac{NS} remnant consists of a
...
and a cooling of the inner regions. 
%
Additionally, the dynamical instabilities in the remnant inject energy and 
angular momentum into the disk, -- a process that manifests itself in the 
form of spiral waves, propagating through the disk. 


%%%% Disk Settling down 
Within the conditions of the \pmerg{}, weak processes take place. 
Together with spiral density waves they cool and periodically shock the 
fluid, bringing the disk to a configuration with an overall smooth 
temperature profile 
...
accreted on a dynamical timescale, shrinking the disk and reducing its total mass by half
...
Whether ordered, large-scale \acp{MF} can form in a \pmerg{} environment 
via dynamo processes is presently unknown. They are important in producing 
...
%%%% Neutrinos
The primary cooling mechanism in the post-\ac{GW}-dominated phase is the emission of 
...
The neutrinos are radiated on the diffusion timescale \citep{Perego:2014fma}.
%\begin{equation}
...
%as chemical potential $\mu_n-\mu_p+\mu_e<0$ at high temperatures, 
...
%
Within the disk, however, the optical depth for neutrinos is ${\simeq}1$, 
allowing neutrinos to diffuse out on a timescale of milliseconds, 
lowering the disk temperature 
...
High temperatures lead to electron-positron pair creation, 
%% [added from Shibata Review paper]
facilitating positron capture by free neutrons 
%$n+e^+ \rightarrow p +\bar{\nu}_e$ 
in the disk.
%
The average energy of a particle is large, close to the mass difference between 
a neutron and a proton 
%The entropy per baryon varies between $3$ and 
%$\lesssim  10\,$$k_{B}$/baryon 
\citep{Perego:2019adq}. 
In combination with the high $\nu_e$ and $\bar{\nu}_e$ luminosities, the large number 
of available positrons leads to an increase in the average charge per baryon, 
\ie, electron fraction $Y_e$\footnote{
    $Y_e = 1/(1 + N_n/N_p)$, where $N_n$ and $N_p$ are the total proper 
    number density of neutrons and protons respectively 
}, of the fluid in relation to the initially 
neutron-rich material \citep{Qian:1996xt}.
%
This process is called deleptonization. %\citep[\eg][]{Perego:2014fma,Endrizzi:2019trv}

Additionally, a \ac{NS} remnant itself is a major emitter of neutrinos. 
...
This drives the neutron and proton fraction towards an equilibrium, 
raising the $Y_e$.
%\red{paper says: 
...
%%%% EOS 
The \pmerg{} dynamics and, ultimately, the remnant's fate, depend strongly on the 
...
The \ac{EOS} effects on the system's dynamics can be quantified with $\tilde{\Lambda}$, 
and it is common to discuss the \ac{EOS} in this context as being 
\textit{softer} or \textit{stiffer} if the $\tilde{\Lambda}$ is 
larger or smaller (we discuss this more in Sec.~\ref{sec:nr_methods:eos}). 
%
...
$M_{\rm max}^{\rm TOV}$, 
and finite temperature and non-equilibrium\footnote{ 
    Beta-equilibrium is the condition when $\mu_n = \mu_p+\mu_e$, where  
    $\mu_n$, $\mu_p$, and $\mu_e$ are the chemical potentials of neutrons, 
...
\ac{BH} formation directly at merger is usually referred to as \ac{PC}. 
...
against collapse by fast rotation  \citep{Baumgarte:1999cq,Rosswog:2001fh,Shibata:2006nm,Bernuzzi:2015opx}.
Its lifetime depends on the \ac{EOS}, finite temperature effects and viscosity,
...
(ii) \ac{SMNS} if $M_{\rm max}^{\rm TOV} < M <  M_{\rm max}^{\rm RNS}$,
and 
(iii) stable \ac{MNS} if $M < M_{\rm max}^{\rm TOV}$ \citep[\eg][]{Baumgarte:1999cq}.
...
even after reaching rigidly rotating configuration. The lifetimes of a \ac{HMNS} and 
a \ac{SMNS} depend on the efficiency of mass and angular momentum loss 
...
%%%% DYNAMICAL EJECTA
Tidal interactions and shocks exerted upon \acp{NS} at merger trigger the 
ejection of material on a dynamical timescale. This is called \ac{DE} \citep[\eg][]{Hotokezaka:2013b,Bauswein:2013yna,Radice:2016dwd,Radice:2018pdn},
composed of a tidal and shocked components as follows. 
%
Shortly before and during a merger, the outer parts 
of the \acp{NS}, opposite to the collisional interface, are stripped 
away by tidal torques and centrifugal forces, forming the tidal 
component of \ac{DE}. This is more massive in binaries with a
...
direction. This comprises the shocked component of \ac{DE} 
...
\ac{DE} has a broad distribution in terms of composition, 
velocity and mass, that is dependent on the parameters of the binary and 
\ac{NS} \ac{EOS}. 
%
However, due to large numerical and systematic uncertanties, 
the relation between the binary parameters and ejecta properties 
is still largely unconstrained \citep{Dietrich:2016fpt,Kruger:2020gig}. 

\ac{NR} simulations show that within the velocity distribution of \ac{DE}, 
there is ${\sim}(10^{-6}-10^{-5})\,\Msun$ of matter ejected at ${\sim}0.8\,c$ 
...
This sometimes referred to as the fast tail of \ac{DE}.
%This matter is sufficiently fast to avoid the neutron capture on seed nuclei 
%%(see Sec.~\ref{sec:intro:nucleo}) 
%and will eventually undergo free-neutron decay, emitting 
%\ac{UV} radiation on a timescale of hours \citep{Metzger:2014yda}. 
...
%
Furthermore, strong neutrino irradiation can drive low-mass outflows,
so-called neutrino-driven winds (\nwind{}; \citet{Dessart:2008zd,Perego:2014fma,Just:2014fka}). 
%
Winds and outflows occurring on a secular timescale sometimes 
referred to as secular ejecta.


The presence of the remnant modifies properties of the ejected material by means of 
neutrino irradiation \citep{Fernandez:2015use}, providing a possibility to infer the nature of 
the remnant from \ac{EM} observations. Modeling this process, however, requires very long-term 
$3$D ab-initio \ac{BNS} merger simulations with complete physics, that are currently unavailable.
%
Overall, analytical estimates and simulations with various approximations show that 
up to ${\sim}40\%$ of the disk can be ejected via viscous processes with a 
...
The maximum $A$ of a nuclide is limited by its binding energy, $Q_n$, as 
...
and depending on whether the timescale for the decay is slower 
or faster than the timescale of neutron capture, one can distinguish between rapid and slow processes respectively,  where the former is called \rproc{} and the latter is called \sproc{}.
%
The \sproc{} moves along the valley of stability\footnote{
    The valley of stability is the region in nuclides chart 
    where nuclides are stable to radioactive decay based on 
    their binding energy. 
}, while the \rproc{} moves along the 
neutron drip line. 
...
that are located at the intersection between the neutron drip line and the closed 
...
%%%% R-process mechanism
The outcome of the \rproc{} \nuc{} depends strongly on the hydrodynamic properties 
of matter and its composition. 
A particularly important quantity is the ratio between the number of free neutrons and 
the number of seed nuclides: the neutron-to-seed ratio. 
%Such parameterized model of \citet{Lippuner:2018phd} (see their Figure 3.1) 
%allows to deduce several important trends.
%
At high entropy, where the neutron-to-seed ratio is high, 
even if seed nuclei are light, \textit{full} (or \textit{main}) \rproc{} 
can occur, producing elements at abundance peaks at $A = 130$ and $195$.
%
...
The \nuc{} that results in the production of only the lightest \rproc{} nuclei, 
up to $A \sim 125$, is generally referred as \textit{weak} \rproc{}. 
%
However, at high entropy and low $Y_e$ the $3$rd peak elements can be synthesized because, 
while there are few seed nuclei, the neutron-to-seed ratio is high.
%
It was pointed out that the main quantity defining the final abundances is  
electron fraction, $Y_e$ \citep{Lippuner:2015gwa}.

%%%% R-proces cites
Conditions for the \rproc{} can be achieved in different astrophysical sites, \eg, 
certain types of \acp{SN} and \ac{BNS} mergers where very neutron rich (\ie, low 
$Y_e$ fraction) conditions can be reached
...
fraction, which, depending on the ejecta velocity, can reach 
${\leq}0.45$ \citep{Qian:1996xt}. 
%
At this point the equilibrium sets in between the ejecta and neutrinos.
High electron fraction implies that only light elements will be produced.
...
viscous- and recombination-driven winds. 
%These processes are expected to take 
%place over long timescales, ${\simeq}1\,$s. 
Studies have shown that these ejecta have a broad distribution of $Y_e$, and the \rproc{}
\nuc{} within it produces light as well as heavy elements
\citep[\eg][]{Just:2014fka,Wu:2016pnw,Fernandez:2018kax}. 
%\citep{Fernandez:2013tya,Just:2014fka,Wu:2016pnw,Siegel:2017nub,Fujibayashi:2017puw,Fernandez:2018kax}.
The production of very heavy \rproc{} elements, however, might be supressed in 
these ejecta if a long-lived \ac{NS} remnant is present 
\citep{Metzger:2014ila,Lippuner:2017bfm}.

%% --- other types
Regarding the \acp{SN}, winds driven by strong neutrino fluxes from the hot, deleptonized
core \citep{Qian:1996xt} were suggested as a promising site of \rproc{} \nuc{}  \citep{Woosley:2002,Wanajo:2006mq}.
However, high $Y_e$ found in such winds does not allow for the full \rproc{} 
and only ``light'' heavy nuclide, up to $A\sim130$, can be synthesized 
\citep[\eg][]{Qian:1996xt,
    %Thompson:2001ys,Fischer:2010,Roberts:2010,
    MartinezPinedo:2012rb,Wanajo:2013} 
%
...
initiated by \ac{MRI} and accompanied by the % magnetorotational processes \eg, 
formation of a collimated bipolar jet 
\citep{Wheeler:2000,Akiyama:2003,Burrows:2007yx,Mosta:2014jaa,Mosta:2015ucs,Siegel:2019mlp}.
...
%%%% Fission cycling
Under very neutron-rich conditions, 
nuclides beyond $A=300$ can be produced. Being unstable to fission, they 
...
point towards a source that was active in the early Universe, which is in tension with the long, 
...
%
Observations of certain \acp{UFG} suggest that stars there have been enriched by rare, 
...
magnetorotationally driven \acp{CCSN}, the compact object mergers can account for the 
...
radioactive decay of material enriched with \rproc{} elements,
ejected in \ac{BNS} or \ac{NSBH} mergers, can power an \ac{EM} transient.
%
The authors showed that, contrary to the normal \acp{SN}, 
the ejecta would quickly become transparent to its own emission, 
which would reach its peak on a timescale of around a few days. 
%
The main difficulty in this pioneering work was the lack of \nuc{}
models to estimate the radioactive heating in ejecta. 
...
%% OPACITIES
%%-----------
The main components of the \ac{kN} modeling include: 
%\begin{itemize}
%\item
...
%\item 
(iii) dominant sources of energy within ejecta % , heating rate %$\dot{Q}(t)$, 
and how efficiently this energy thermalizes.
%\end{itemize}
%
...
(i) free-free transitions, 
(ii) bound-bound transitions, 
(iii) certain optically thick lines, 
(iv) ejecta clumping, 
...
%
When considering frequency-integrated intensities, 
frequency independent, gray opacities are employed.  
For example, Plank mean opacity is used when computing 
integrated thermal emission for an optically thin plasma.
% For example, Rosseland mean opacity is used in optically thick medium 
% and Plank mean opacity for the
% integrated thermal emission for an optically thin plasma
...
The calculation of mean opacities, however, still requires complex 
atomic models. 
%
...
However, it becomes increasingly inaccurate, as lines broaden 
...
Generally speaking, calculations of \ac{kN} emission are complicated by the %ejecta opacity, 
%as there is a 
lack of experimental data and numerical models of the optical opacity 
...
complex atomic structures \citep{Kasen:2013xka,Tanaka:2013ana}. 
%
Higher opacities shift light curve peak time by ${\sim}1\,$week \citep{Barnes:2013wka} 
and shift the spectral peak from optical/\ac{UV} to \ac{NIR}.

%% ----------------------------
%% \paragraph{\rproc{} heating}
%% ----------------------------
The heating in ejecta occurs through a combination of $\beta$- and $\alpha$-decays, 
and fission \citep{Metzger:2010,Barnes:2016umi,Hotokezaka:2017dbk}. 
%
Decay products then thermalize with a certain efficiency % $\varepsilon_{th,\upsilon}$, 
that depends on interactions between them and the thermal plasma. 
Notably, neutrinos escape the ejecta freely. 
Very high energy photons (gamma rays), are also free after about 
...
%
Additionally, if actinides are synthesized in the \rproc{}, their decay products, \eg,
$\alpha$-particles, thermalize with high efficiency. 
%\red{double check with before, Barnes}. 
% Nuclear physics inputs that determine the amount of actinides, 
...
%absorption lines \citep{Tanaka:2013ana,Kasen:2013xka}.
%
The properties of the \ac{kN} emission have strong 
dependency on the properties and geometry of 
...
%% from remnant section
Notably, \ac{BNS} mergers that experience \ac{PC} are not expected to eject large 
amounts of material and are generally thought of as being \ac{EM}-quiet 
\citep{Margalit:2017dij,Bauswein:2017vtn}. 
From \ac{EM} observations of a merger, the fate of the remnant can be inferred, 
albeit in a model-dependent way. 

%% --- Modelling
To model the \ac{kN} emission robustly, time- and energy-dependent 
photon radiation transport models are required 
\citep{Kasen:2017sxr,Tanaka:2017qxj,Miller:2019dpt,Bulla:2019muo}.
%
Meanwhile, semi-analytic one- or multi-component spherical \ac{kN} models 
are often employed due to their simplicity and high computation speed.
%
Notably, systematic uncertainties in
nuclear physics (\eg, mass models, fission fragments and $\beta$-decay
rates) and atomic physics (\eg, detailed wavelength dependent opacities for
\rproc{} elements) enter all the current \ac{kN} models 
\citep{Eichler:2014kma,Rosswog:2016dhy,Gaigalas:2019ptx}.
%
%The properties of the original \ac{BNS} can be inferred by fitting the 
%ejecta properties obtained with \ac{kN} models, 
%\red{from KN section}
...
Our understanding of \ac{kN} has significantly improved after 
the detection of \AT{} \citep[\eg][]{Metzger:2019zeh}.
%
Both ``blue \ac{kN}'' and ``red \ac{kN}'' were observed, 
...
%secular ejecta. 
%Semi-analytic two-component spherical \ac{kN} models to 
%the \AT{} observations provided estimates for the ejecta properties. 
%%for these two components. 
%%
%For the langhinide poor (rich) \ie, blue (red) ejecta components, 
...
%
The presence of \ac{EM} counterparts strongly disfavors \ac{PC} in the case of \GW{} 
\citep{Margalit:2017dij,Bauswein:2017vtn,Radice:2017lry}. However, the 
question of whether it was \ac{HMNS} or \ac{SMNS} remains open 
...
merger simulations \citep[\eg][]{Kawaguchi:2018ptg}. % see also Sec.~\ref{sec:intro:kilonova}. 
%
...
For instance, the required amount of low-$Y_e$, material 
...
low opacity and massive ejecta, that is not generally found in \ac{NR} 
simulations \citep{Fahlman:2018llv}. 
%% ---
Notably, the early blue component can be explained by the emission arising 
from the interaction between a relativistic jet and the ejecta
\citep{Lazzati:2016yxl,Bromberg:2017crh,Piro:2017ayh}.
However, simulations show that successful jets do not deposit a sufficient amount of thermal energy in ejecta for this mechanism to work \citep{Duffell:2018iig}. 
...
These explanations, however, require the development of large-scale strong 
magnetic fields.

%% --- Kilonova
Prior to \AT{}, there were other \ac{kN} candidates based on the 
detection of \acp{SGRB}, with infrared excess \eg, 
GRB130603B, \citep{Berger:2013wna,Tanvir:2013pia}, 
GRB060614 \citep{Jin:2015txa,Yang:2015pha}, 
GRB050709 \citep{Jin:2016pnm}.
%GRB200522A \citep{WRONG}  Bruni:2021ilp
...
Observed since 1967, \acp{GRB} are irregular pulses of gamma-ray radiation with 
broken power-law (non-thermal) spectrum, peaking at KeV-MeV 
\citep{Band:1993,Kouveliotou:1993,Meegan:1992xg}.
%
...
that allow the estimation of redshift \citep[\eg][]{Costa:1997cg,Frontera:1997ae}.
...
when \ac{GRB}-ejecta sweeps-up the \ac{ISM} medium\footnote{
    ...
    are the power law decay of light curves, and the power-law spectrum. 
...
for at least some \acp{SGRB}, it was dimmer than any other event of its class. 
...
The unusual signature of \GRB{} is now commonly attributed to the
fact that it was a structured jet\footnote{
    A structured jet, contrary to a top-hat jet, has an angular dependency of the 
    matter energy and Lorentz factor. Such structure is believed to appear 
    when a jet drills its way through the merger ejecta \citep{Lamb:2017ych}.
}, which was also observed off-axis \citep[\eg][]{Ghirlanda:2018uyx}. 
...
emission is expected to arise from 
%electrons accelerated in shocks formed between the 
mildly relativistic ejecta expanding into the \ac{ISM} \citep{Nakar:2011cw}. 
Called \ac{kN} afterglow, it is expected to peak in radio band and continue 
to be observable on a time scale of years 
after a merger, providing a source of information that is largely free 
from uncertainties in the nuclear and atomic physics of \ac{kN} models. 
%
Notably, all ejecta components contribute to the emission, 
...
interact with each other and with the \ac{ISM}, a
long-lived \blast{} is generated.  
The formed shock, propagating upstream, amplifies random magnetic fields and accelerates 
electrons that subsequently emit synchrotron radiation \citep{Kumar:2014upa}. % and \ac{SN} early remnants. 

As \ac{NR} simulations showed the presence of mildly relativistic ejecta, 
\eg, from core \bnc{}s, such emission was studied in previous works 
...
Since ${\sim}160\,$days after the merger the non-thermal emission from \GW{} has been 
...
While the exact nature of this change remains at present unclear, one 
of the possible explanations is the emergence of the 
\ac{kN} afterglow\footnote{
    See, however, \citet{Troja:2021xsw} for an alternative explanation
} \citep{Hajela:2021faz}.
...
\ac{BNS} mergers are at the center of a variety of 
...
%
The foundation of \ac{BNS} merger research are \ac{NR} simulations.
However, most \ac{BNS} \ac{NR} simulations available in the literature 
are either short, typically ${\lesssim}10\,$ms \pmerg, but with 
...
\ac{NR} simulations performed using \wisky{} \ac{NR} code.
Our simulations include \ac{GR}, microphysical \acp{EOS} with finite-temperature 
effects, neutrino emission and reabsorption, and effects of 
...
\GW{}. 
%
Postprocessing and analyzing these simulations, 
...
%    the resulted nucleosynthetic yields?
    Based on the abundance pattern in ejecta, can \ac{BNS} mergers be considered 
    the prime source of \rproc{} elements in the Universe?
    \item How do the statistical properties of \ac{DE} of our simulations 
    ...
    \item Can the \ac{kN} produced by ejecta from our long simulations 
...
Ultimately, we aim to find a possibility for further constraining 
the properties of \GW{} and \ac{NS} \ac{EOS}.
...
%
In Chapter \ref{ch:nr_methods} we provide a brief overview 
of the methods used to model \ac{BNS} mergers and implemented in \wisky{},
and techniques we used to postprocess these simulations. 
%
In Chapter \ref{ch:bns_sims} we present the results of simulation 
postprocessing and discuss the \pmerg{} dynamics and ejecta.
%
In Chapter \ref{ch:nucleo} we overview a method to compute \rproc{} 
\nuc{} final abundances in ejecta, and compare these abundances in ejecta 
from our simulations with solar ones. 
%
In Chapter \ref{ch:kilonova} we overview a method to compute \ac{kN} emission. 
Then, we apply a previously developed model to ejecta from our simulations  
and compare the result with observations of \AT{}.
%
In Chapter \ref{ch:afterglow} we overview a method to compute synchrotron 
radiation from ejecta, focusing on particular techniques that we implement 
numerically. Then we apply our new tool to the ejecta from our simulations and 
compare the result with the \GRB{} afterglow.
%
