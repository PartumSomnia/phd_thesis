\chapter{Methods of BNS merger simulations}\label{ch:nr_methods}
...
%\red{
%    Remove what 'our' simulations use from each section, and say
%    it once in the end of the chapter
%}
...
\begin{figure}[t]
    \centering 
    \includegraphics[width=0.70\textwidth]{tim_3p1_plot.pdf}
    \caption{
...
The transition between the two hypersurfaces, $\Sigma_t$ and $\Sigma_{t+dt}$,
...
%Gauge conditions describe the specific foliation of the spacetime, 
...
In our simulations, so-called ``1+log'' slicing is used. %, when $f(\alpha) = 2/\alpha$.
%
...
The set of constraint equations is derived by utilizing two relations.
...
Consider classical density, scalar $\rho$, usually defined as the 
...
%\end{equation}
%%% ------------------------------------------------------------------------
The stress-energy tensor of a perfect fluid in the comoving frame 
reads 
%
\begin{equation}
T^{\mu\nu} = \rho h u^{\mu}n^{\nu} + p g^{\mu\nu}\, ,
...
\section{The neutron star equation of state}\label{sec:nr_methods:eos}
...
In order to close the system of hyperbolic equations that include \ac{GRHD} 
...
%
%All \acp{EOS} models have symmetry energy at saturation density that is in 
...
% Then, the nucleon single particle potentials %(via integration of the on-shell effective interaction matrix),
...
%
%This \ac{EOS} was first employed for the \ac{BNS} merger 
...
\begin{figure}[t]
    \centering 
    \includegraphics[width=0.70\textwidth]{tov_mr.pdf}
    \caption{
...
%, and their relation to the electron fraction, 
we consider 
...
$\Mmax$ and $R_{1.4}$ are related to pressure at half saturation density \citep{Lattimer:2012nd}.
...
Most commonly used approaches to simplify radiation transport rely 
...
\begin{table}
    \caption{
        ...
        % In the left column, $\nu \in \{\nu_e, \bar{\nu}_e, \nu_{x}\}$ denotes any neutrino species, where $\nu_{x}$ refers to heavy-lepton neutrinos, 
        % $N \in\{n, p\}$ a nucleon, and $A$ any nucleus.
...
It is numerically advantageous to treat neutrinos in the optically thick regime, 
...
%%% ==== SCATTERING
In addition to the emission of neutrinos, the leakage scheme considers neutrino absorption 
...
The turbulence tensor can, in turn, be expressed as a function of 
...
A subset of our \ac{BNS} merger simulations was computed 
...
\section{Simulations and postprocessing} \label{sec:nr_methods:sims}
...
%
%The details on the numerical setup of \wisky{} 
%are reported in Appendix~\ref{app:whisky}.
%
...
%
% and summarize their main properties, 
...
%\citet{Radice:2016dwd,Radice:2018pdn}, that used the same code and similar 
...
%corresponding to the source of \GW{}. 
...
%
%The extraction time, \ie, the time \pmerg{} when the disk mass is computed, 
%is also important, but less so, as we find that the accretion on the 
...
%of the spacetime's $3+1$ decomposition.
%
...
(\ie, fluid's pressure gradient, internal energy and heating) %(\eg, due to an $r$-process (\red{see section XXX})) 
...
%which is defined up to a constant factor. 
%
...
%, it was reported that when Bernoulli criterion is used, 
