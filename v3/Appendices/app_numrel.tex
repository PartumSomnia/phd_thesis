...
\red{source called them 'coservational laws' -- but there seesm to be no such term}
...
In Section \red{fill me, if it is needed for just a Section?..}
...
In this \red{subsection} we briefly recall the basics behind the mathematical theory of conservation laws and subsequently, their numerical application. 
...
%% Analytical properties and requirements for conservation laws
...
\red{Write a better definition of an entropic solution to conservation laws}
...
%% Numerical treatment of the conservation laws
...
%That characterizes the Lebesgue outer measure. Whether this outer measure translates to the Lebesgue measure proper depends on an additional condition. 
...
%% constructing a numerical sheme [INTRO]
...
%% Deriving the numerical scheme that is convergent and leads to an entropic solution
...
Multiple other high order accuracy schemes have been proposed with however weaker conditions of non-linear stability and not yet fully explored convergence. 
...
%In this \red{section} we consider the basic Godunov method for the first order schemes. 
%Then, we iterate on the second-order Godunov-type schemes, that are the most widely used in the field of relativistic hydrodynamics. 
%In the end of the subsetion, we elaborate on the extension of FV methods to even higher orders, and pint out the difficulties arising in application to the relativistic hydrodynamics


\paragraph{The Godunov Method}


\textcolor{red}{my overall understanding of this method is rather poor and thus I copied David. It needs to be revised more.}

%% Introduction on the derivation process and properties of Godunov Scheme
...
%% Deriving FV, the Godunov Scheme
...
Averaging the advection equation \ref{eq:theory:fv:adveq} over one control volume and one time-step we obtain
...
To compute \ref{eq:theory:fv:semi-discrete}, a time integrator has to be chosen. 
...
\paragraph{TVD Finite-Volume Methods}


%% On the reconstruction procedure from FR averages
...
The PPM reconstruction is based on the piece-wise parabolic interpolation, that extends the slope-limiter approach. 
...
The basic WENO reconstruction yields the smooths solution, utilizing the "smoothness indicator", that, in case of the smooth solution maximizes the order of accuracy, while minimizing it, when the discontinuities are detected. 
...
A second order central scheme was developed by Nessyahu and Tadmor (NT) \cite{Nessyahu:1990}. Kurganov and Tadmor (KT) \cite{Kurganov:2000} have further advanced the scheme, which popularized high-order central schemes. 
...
which is identical to the semi-discrete form of the finite volume scheme introduced before \ref{eq:theory:fv:1storder3dscheme}. 
...
and there were no approximations done in writing \ref{eq:theory:fd:introd_h} and \ref{eq:theory:fd:diff_via_h}.
...
The ones that are of relevance to \red{this thesis???} are: \textit{Roe flux-split}, \textit{i.e.,}
...
With respect to the efficiency, the Roe flux splitters appear superior, as it requires only one reconstruction. However, it was shown \cite{LeVeque:1992} that, if the trans-sonic refraction waves \red{not transonic rarefaction waves?} are present, this flux splitter can lead to the formation of entropy-violating shocks. 
...
For simplicity, let us consider a one-dimensional case $\alpha={0,1}$. 
...
and we perform component-wise reconstruction to obtain $Q_{1+1/2,j,k}$, where $\omega$ is used in place of $u$ and $Q$ in place of $f$ in the \ref{eq:theory:fd:laxfluxsplit}. 
...
\paragraph{Runge-Kutta Discontinuous-Galerkin Methods}



In this \red{paragraph} we are going to briefly touch on the fundamentals behind the so-called spectral discontinuous Galerkin method with numerical integration (SDGM-NI) of the nodal-DG scheme, which is the widely used DR variant. 
...
Assuming that $\boldsymbol{f}$ is smooth 
...
which are the \textit{flux vectors}.
...
\red{David has it as a chapter, introduction to his THC code}
\red{These are meant as an introduction before tests on Neutownian and SR hydro were performed}
...
\red{note that in the GRB code we set it as $\hat{\gamma}$}
...
In relativity it is common to adopt the unit system in which $c=1$. 
