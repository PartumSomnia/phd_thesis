...
Considering the mass-averaged ejecta velocity, $\avd$, of the \DSrefset{}, we find that it in 
...
%% Residual plots
We compare the $\ayd$ from datasets and predicted by fitting formulae in Fig.~\ref{fig:ejecta:dyn:y} 
...
%        Best fitting coefficients are given in the tables in Appendix~\ref{app:coefs}.
%        Here $\Delta M_{\rm disk} = M_{\rm disk} - M^{\rm fit}_{\rm disk}$.
%        The fitting procedure here was based in minimizing residuals instead of $\chi_{\nu}^2$ as otherwise, the error measure adapted, Eq.~\eqref{eq:disk:mdisk_err}, would lead to the 
