...
One of the widely used variants of DG method is the spectral discontinuous Galerkin methid with numerical integration (SDGM-NI) of the nodal-DG scheme \cite{Hesthaven:2007}. Here we will take a brief look at its main principles, and how in a classical way semi-discrete scheme can be derived. We consider a one-dimensional hyperbolic equations, the advection equation with no sources, for simplicity.
...
where $\Omega\subset \text{I\!R}^{d}$ is a bounded, regular domain. In this domain, $\Omega$, we perform a triangulation $\mathbb{T}$, such that the domain is now is a union of $N$ images of a reference element, $T$, \textit{i.e.,} $\cup_{j=1}^{N}\Omega_{j} = \Omega$ and $\Omega_{i}\cap\Omega_{j} = \emptyset$, that is usually a triangle in 2D and tetrahedron in 3D. This mapping allows to "pull-back" the equations from a given element of arbitrary complexity in physical space, into a reference one, where all the discrete differential operators, derivatives, interpolation and integration have been pre-computed once for a simple geometry. This is the key idea behind the DG scheme. In relativistic case, a covariance of the scheme can be "combined" with the covariance of equation to render effectively coordinate-free. 

Next we will briefly outline how a weak formulation of \ref{eq:theory:dg:adveq} is obtained \gray{David comments on a diferent way for GRHD in section 6 of his thesis}. We assume that $u$ is a  bounded function in space and a smooth function in time.
