...
where $t_{\text{exp}}$ is the time since mass ejection, $\lambda_{c}$ is the central wavelength of the bin, $\Delta\labmda_c$ is the bin width, $\tau_i$ is the Sobolev optical depth of a line \cite{(Sobolev 1960)}, and the sum runs over all lines in the bin.
...
The use of Sobolev $\tau_i$ two statements are assumed:
...
Notably, withing the expanding ejecta the first criterion can be satisfied. However, the second may not. In \cite{Kasen et al. 2013} the example of a more rigourous radiation tranport is considered with non-Sobolev opptical deoth of lines.
