...
Notably, only a subset of particles is evolved by a reaction network, whyle for others the conditions are assumed to be unchanged. For instance, photons are assumed to be in equilibrium, while electron and positron densities are set by the charge neutrality. 
Describing particle interaction and nuclide transmutation it is common to introduce the entrance and exit channels. Then, reaction rate is defined as a speed at which a reaction proceeds per particle in the entrance channel. Thus, if there is no change between particles in entrance and exit channels, the RR is zero. A particular usefull quantity is \textit{abundance} $Y_i$, defined as 
...
where $\lambda_{\alpha}$ is the reaction rate of the forward process, $R_{i}^{\alpha}$ are reactants, $P_{i}^{\alpha}$ are products, $N_{i}^{\alpha}$ number of particles of the species $i$ involved and $Y_m^{N_{m}^{\alpha}}$ are the abundances of the particles of psecies $i$ involved. (e.g., Hix and Thielemann, 1999)
