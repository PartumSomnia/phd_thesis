...
The key component of a nuclear reaction network are the reaction rates (RR) for various processes. 
In general RR depend strongly on the particle energy, intermediate states of compound nucleus reaction (resonances) (\textit{e.g.,} \cite{Clayton:1968}, \textsection{4}), while the single-nuclide reactions, \textit{e.g.,} $\beta$-decay, are assumed to have constant RR, which is strictly speaking is true only in vacuum.
For many nuclides formed far from the valley of stability these rates are not experimentally known and need to be modeled \cite{Cyburt:2010}. This is usually done by following a subsequent decay of a single compound nuclides, formed in the reaction \cite{Hauser:1952,Rauscher:2000fx,Goriely:2008zu}.
