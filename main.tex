...
However, the question of weather BNS (NSBH) is a dominant cite or $r$-process nucleosynthesis remains open \cite{Qian:2000bh,Argast:2003he,Matteucci:2014}. In particular as it is difficult to explain the observed composition of very metal-poor stars (MP) and a rather uniform $r$-process abundance patters in stars in the Galaxy. 

After the Big Bang nucleosynthesis, the Universe consisting of hydrogen and helium,  with traces of lithium, have expanded and cooled. Under he influence of dark matter, the primordial gas fragmented and first stars, galaxies and galaxy clusters have formed. During their lifetime the first stars (population III stars) converted light elements into heavier ones and then ejected them back into the interstellar medium (ISM) during supernova events. Thus, future populations of stars thus were born of gas enriched with heavy elements, in particular, iron. As the amount of elements heavier then hydrogen and helium is stars (\textit{i.e.,} metallicty) increases with each stellar generation and an age-metallicity relation can be derived \cite{Matteucci:2012}. And stars spectroscopic surveys \footnote{And indicative quantity of metallicty measured in such iron-to-hydrogen ration, [Fe/H],that reads as a $\log_{10}$ of the abundance of a element $X$ to hydrogen, normalized to solar ration, \textit{i.e.,} in the sun for every X, [X/H]$= 0$. If a stars has [Fe/H]$=-2$, it is said that this star ahs a 100 times less iron compared to hydrogen then sun.} provide information on chemical evolution of the Galaxy \cite{Edvardsson:1993,Suda:2008na}. Notably, multiple dark matter sub-halos contributed to the formation of the Galaxy, it is not expected that there is a unique age-metallicity relation (see \textit{e.g.,} \cite{Ishimaru:2015} and references therein) 

While the enrichment of interstellar medium with heavy elements from stellar interior occurs immediately after stars die, the $r$-process elements, produced in BNS (NSBH) mergers can only enrich ISM when compact objects inspiral and merge which on average takes $(0.1-1)\times10^{9}$ years \cite{Dominik:2012kk}. 
